...
% ---- Legend (placed below the whole diagram, full width) -------------------
\node[draw=black!30, rounded corners=2pt, inner sep=3pt,
      below=6mm of pred.south west, anchor=north west,
      fit={(pred.south west) ([xshift=-6mm]uq.south east)}] (legendbox) {};
\node[anchor=west, font=\scriptsize, inner sep=0pt] (leg-label)
      at ([xshift=2mm]legendbox.west |- legendbox) {\textbf{Legend}};

% Build the three legend items horizontally, left to right.
...
\node[box, fill=colTrainable, draw=colTrainableEdge,
      minimum width=5mm, minimum height=3.2mm, inner sep=0pt,
      right=14mm of sw1.east, anchor=west] (sw2) {};
\node[anchor=west, font=\scriptsize, inner sep=1pt]
      at ([xshift=0.8mm]sw2.east) {trainable capsule head (295 K)};

\node[lora, minimum size=3.2mm, right=34mm of sw2.east, anchor=west] (sw3) {L};
\node[anchor=west, font=\scriptsize, inner sep=1pt, text=colLoRA!80!black]
